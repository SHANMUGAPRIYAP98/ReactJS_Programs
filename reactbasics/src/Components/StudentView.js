...
export default function StudentView(props) {
    return (<div>
           ...
           <Table striped hover size="sm" id="stud">
            ...
            <th>PHONE NO</th><th>ADDRESS</th><th>BLOOG GRP</th><th>DEGREE</th></tr></thead>
            <tbody><tr><td>{props.data.regno}</td><td>{props.data.sname}</td><td>{props.data.mail}</td>
            <td>{props.data.phone}</td><td>{props.data.addr}</td><td>{props.data.bg}</td><td>{props.data.deg}</td></tr></tbody>
           </Table>
           </div>
    )
}
